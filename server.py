...
def quantize_16(img: Image.Image):
    # Dither disabled keeps the 2-color cell cleanup steadier and faster-looking.
...
def fast_enforce_two_colors_per_cell(indexed, mon_w=164, mon_h=81):
    """
    FAST version:
    each 2x3 cell keeps its two most common palette indices.
    Any extra colors get replaced by whichever of the two colors is more common.

    This is less perfect than the slow RGB best-pair method, but it is vastly faster
    on Render free and still guarantees blit legality.
    """
    ...
    for cy in range(mon_h):
        ...
        for cx in range(mon_w):
            ...
            # Extra colors become keep_b, preserving the most common as bg.
            for dy in range(3):
                for dx in range(2):
                    v = out[y0 + dy][x0 + dx]
...
def make_thumbnail_frame_chunk(job: str, video_id: str, title: str, thumb_url: str = "") -> str:
    ...
    try:
        img.save(CURRENT_DIR / "thumbnail_source_328x243.jpg", quality=80)
    except Exception:
        pass
    ...
    # Debug timing for Render logs.
    print(
        f"[encoder] thumbnail={t1-t0:.3f}s quant={t2-t1:.3f}s enforce={t3-t2:.3f}s pack={t4-t3:.3f}s total={t4-t0:.3f}s",
        flush=True,
    )
    ...
    lines.append("source=thumbnail_fast")
    lines.append("fps=10")
    lines.append(f"w={w}")
    lines.append(f"h={h}")
...
def root():
    return {
        ...
        "chunk_mode": "youtube_thumbnail_fast_one_frame",
        "try": [
            "/ping",
...
def prepare(id: str, title: str = "", thumbnail: str = ""):
    ...
    manifest = {
        ...
        "message": "Thumbnail image was encoded into one fast CCV frame.",
    }
