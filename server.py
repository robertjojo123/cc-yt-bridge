...
def make_multiframe_thumbnail_chunk(job: str, video_id: str, title: str, thumb_url: str = "", frames=30, fps=10) -> str:
    ...
    lines.append("source=thumbnail_motion_async")
    ...
    for i in range(frames):
        set_status(status="processing", ready=False, progress=i, total=frames, message=f"Encoding frame {i + 1}/{frames}")
    ...
    t1 = time.time()
    print(f"[encoder] async multiframe thumbnail frames={frames} total={t1-t0:.3f}s", flush=True)

    return "\n".join(lines)


def background_encode_current(job: str, video_id: str, title: str, thumbnail: str, frames: int, fps: int):
    try:
        chunk_name = "chunk_0001.ccv"
        chunk_rel = f"current/video/{chunk_name}"
        chunk_path = CURRENT_VIDEO_DIR / chunk_name

        set_status(status="processing", ready=False, progress=0, total=frames, message="Starting thumbnail motion encode")
        ...
        manifest = {
            ...
            "source": "youtube_thumbnail_motion_async",
            "fps": fps,
            "w": 164,
            "h": 81,
            "subpixel_w": 328,
            ...
            "message": f"Async thumbnail motion test encoded into {frames} frames at {fps} FPS.",
        }
        ...
        set_status(status="error", ready=False, error=str(e), message="Encoding failed")
...
def root():
    return {
        ...
        "storage": "render_temp_local",
        "chunk_mode": "async_thumbnail_motion",
        "try": [
            "/ping",
...
@app.get("/prepare")
def prepare(background_tasks: BackgroundTasks, id: str, title: str = "", thumbnail: str = "", frames: int = 30, fps: int = 10):
    ...
    job_info = {
        ...
        "progress": 0,
        "total": frames,
        "message": "Queued background encode",
        "manifest_url": f"{base_url()}/files/current/manifest.json",
        "status_url": f"{base_url()}/status?job={job}",
        "created_at": int(time.time()),
    }
    write_json(CURRENT_DIR / "job.json", job_info)

    background_tasks.add_task(background_encode_current, job, video_id, title, thumbnail, frames, fps)
...
def manifest(job: str = ""):
    ...
    if not m:
        return JSONResponse(status_code=404, content={
            "ok": False,
            "error": "no_manifest",
            "message": "No current manifest exists yet. Poll /status until ready.",
        })
